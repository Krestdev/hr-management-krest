...
const headerVariants = cva("w-full min-h-24 px-4 flex flex-row flex-wrap items-center justify-between gap-3 rounded-lg", {
  variants: {
    variant: {
      primary:
...
function Header({children, title, variant, className=""}:Props & VariantProps<typeof headerVariants>) {
  return (
    <header className={cn(headerVariants({variant}), className)}>
        <h1 className="text-[clamp(18px,1.5vw,24px)] leading-[120%]">{title}</h1>
        {children}
    </header>
  )
}
